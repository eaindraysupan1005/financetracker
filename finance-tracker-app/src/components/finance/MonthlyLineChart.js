...
const MonthlyLineChart = ({ userId }) => {
    ...
    useEffect(() => {
        ...
        const fetchMonthlyExpenses = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/expense/monthly/categories/${userId}`);
        ...
        const processChartData = (data) => {
            ...
            const months = Array.from({ length: 12 }, (_, i) =>
                new Date(new Date().setMonth(new Date().getMonth() - i)).toLocaleString('default', { month: 'short' })
            ).reverse();
            ...
            const datasets = categories.map((category) => ({
                ...
                data: months.map((month) => data[month]?.[category.charAt(0).toUpperCase() + category.slice(1)] || 0), // Match backend format for category capitalization
                fill: false,
            ...
            {chartData ? (
                <Line
                    ...
                    options={{
                        ...
                        scales: {
                            x: {
                                title: { display: true, text: 'Month', color: 'white',font:{size: 14 } },
                                ticks: { color: 'white' } // Set x-axis tick labels to white
                            },
                            y: {
                                title: { display: true, text: 'Amount ($)', color: 'white',font:{size: 14 } },
