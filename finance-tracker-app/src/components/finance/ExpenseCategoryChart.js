...
const ExpenseCategoryChart = ({ userId }) => {
    ...
    const backgroundColors = Object.keys(categoryData).map(category => categoryColors[category] || categoryColors["Others"]);
    const borderColors = backgroundColors.map(color => color.replace('0.2', '1')); // Make border color more opaque
        ...
        <div>
            ...
            <Bar 
                ...
                options={{
                    ...
                    scales: {
                        x: {
                            title: { display: true, text: 'Category', color: 'white',font:{size: 14 } },
                            ticks: {
                                color: 'white',
                            }
                        },
